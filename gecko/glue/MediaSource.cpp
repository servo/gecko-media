...
// TODO (gecko-media) #include "mozilla/gfx/gfxVars.h"
...
IsWebMForced(DecoderDoctorDiagnostics* aDiagnostics)
{
  bool mp4supported =
    DecoderTraits::IsMP4SupportedType(MediaContainerType(MEDIAMIMETYPE("video/mp4")),
                                      aDiagnostics);
  bool hwsupported = false; // TODO (gecko-media) gfx::gfxVars::CanUseHardwareVideoDecoding();
#ifdef MOZ_WIDGET_ANDROID
...
MediaSource::Constructor(ErrorResult& aRv)
{
  RefPtr<MediaSource> mediaSource = new MediaSource();
...
void
MediaSource::EndOfStream(const MediaSourceEndOfStreamError& aError, ErrorResult& aRv)
{
  MOZ_ASSERT(NS_IsMainThread());
  MSE_API("EndOfStream(aError=%d)", uint32_t(aError));
  if (mReadyState != MediaSourceReadyState::Open ||
      mSourceBuffers->AnyUpdating()) {
    aRv.Throw(NS_ERROR_DOM_INVALID_STATE_ERR);
    return;
  ...
  switch (aError) {
  case MediaSourceEndOfStreamError::None:
...
/* static */ bool
MediaSource::IsTypeSupported(const nsAString& aType)
{
  MOZ_ASSERT(NS_IsMainThread());
  DecoderDoctorDiagnostics diagnostics;
  nsresult rv = IsTypeSupported(aType, &diagnostics);
...
// TODO (gecko-media)  MOZ_ASSERT(!mMediaElement);
...
MediaSource::Detach()
{
  ...
  // TODO (gecko-media) mMediaElement = nullptr;
...
MediaSource::MediaSource()
  : mDecoder(nullptr)
  // TODO (gecko-media)
  // mAbstractMainThread(do_GetMainThread())
  , mReadyState(MediaSourceReadyState::Closed)
{
  ...
  MSE_API("MediaSource mSourceBuffers=%p mActiveSourceBuffers=%p",
          mSourceBuffers.get(), mActiveSourceBuffers.get());
}
...
MediaSource::DispatchSimpleEvent(const char* aName)
{
  ...
  // TODO (gecko-media) DispatchTrustedEvent(NS_ConvertUTF8toUTF16(aName));
}
...
MediaSource::QueueAsyncSimpleEvent(const char* aName)
{
  ...
  // TODO (gecko-media) mAbstractMainThread->Dispatch(event.forget());
}
