...
#define GECKO_TEST_PATH "gecko/test/"
...
public:
  ...
  explicit MP4DemuxerBinding(const char* aFileName = GECKO_TEST_PATH "dash_dashinit.mp4")
...
TEST(MP4Demuxer, CENCFragVideo)
{
  ...
  RefPtr<MP4DemuxerBinding> binding = new MP4DemuxerBinding(GECKO_TEST_PATH "gizmo-frag.mp4");
...
{
  RefPtr<MP4DemuxerBinding> binding = new MP4DemuxerBinding(GECKO_TEST_PATH "short-zero-in-moov.mp4");
  binding->RunTestAndWait([binding] () {
...
{
  RefPtr<MP4DemuxerBinding> binding = new MP4DemuxerBinding(GECKO_TEST_PATH "short-zero-inband.mov");
  binding->RunTestAndWait([binding] () {
    // It demuxes without error. That is sufficient.
    binding->mTaskQueue->BeginShutdown();
  });
...
{
  RefPtr<MP4DemuxerBinding> binding = new MP4DemuxerBinding(GECKO_TEST_PATH "negative_duration.mp4");
  binding->RunTestAndWait([binding] () {
    // It demuxes without error. That is sufficient.
    binding->mTaskQueue->BeginShutdown();
  });
...
void Test_MP4Demuxer()
{
  Test_MP4DemuxerSeek();
  Test_MP4DemuxerCENCFragVideo();
  Test_MP4DemuxerCENCFragAudio();
  Test_MP4DemuxerGetNextKeyframe();
  Test_MP4DemuxerZeroInLastMoov();
  Test_MP4DemuxerZeroInMoovQuickTime();
  Test_MP4DemuxerIgnoreMinus1Duration();
}
